...
//import Calender_.Modify.aaa;
//import Calender_.Modify.bbb;

//import Calender_.Modify.ddd;

//import Calender_.Insert.hhh;

//import Calender_.Modify.MODIFY;
	...
	private class DELETE implements ActionListener {
		public void actionPerformed(ActionEvent g) {
			...
			if((t3.getText()).equals("")) {
				JOptionPane.showMessageDialog(Delete.this, "there is some fields empty", "WARNING", JOptionPane.WARNING_MESSAGE);
				return ;
			}
			 ...
			 String s=t3.getText();
				for(int i=0;i<s.length();i++) {
					if(!(s.charAt(i)>='0'&&s.charAt(i)<='9')){
						JOptionPane.showMessageDialog(Delete.this, "the year should be integer", "WARNING", JOptionPane.WARNING_MESSAGE);
						return ;
						
						}
			            }
			
			if ((Double.parseDouble(t3.getText()))<1970) {
				JOptionPane.showMessageDialog(Delete.this, "this is not valid year", "WARNING", JOptionPane.WARNING_MESSAGE);
			return ;
		}
			...
			for (int i=0;i<CALENDER.array.size();i++) {
				if((start[aa].equals(CALENDER.array.get(i).get_Start_Time()))&&(end[bb].equals(CALENDER.array.get(i).get_End_Time()))&&(days[dd].equals(CALENDER.array.get(i).get_Day()))&&(names[kk].equals(CALENDER.array.get(i).get_Month()))&&(t3.getText().equals(CALENDER.array.get(i).get_Year()))) {   
			
					CALENDER.array.remove(i);
	                y=true;
	                JOptionPane.showMessageDialog(Delete.this, "this task has been deleted successfully", "WARNING", JOptionPane.WARNING_MESSAGE);
	                
				}
			}

			if(!y) {
				JOptionPane.showMessageDialog(Delete.this, "this task hasn't be found", "WARNING", JOptionPane.WARNING_MESSAGE);
